...
(function(){
  ...
  function handleEvent(event){
    if (event){
      ...
      if (event.type === 'connect' && event.socket_id && $.cookie.id_token){
  ...
  function checkkey(event) {
    const path = private.clickEvent ? [...private.clickEvent.path] : [];
		path.push($);
		path.forEach(elem => {
  ...
  function importXlsFile(file) {
		...
		reader.onload = event => {
			...
			function importSheet(sheetname) {
				...
				// const private = wbsheet['!private'].split(':').pop();
  ...
  function handleData(targetItem, event) {
    ...
    if (targetItem) {
      ...
      const type = private.keyEvent && private.keyEvent.shiftKey ? 'link' : event.type;
      let data;
  ...
  function onkey(event) {
		...
		private.keybufferTimeout = setTimeout(() => private.keybuffer = [], 300);
		...
		checkPath(event);
		if (private.clickEvent) {
			event.itemElement = event.itemElement || private.clickEvent.itemElement;
			event.item = event.item || private.clickEvent.item;
  ...
  AuthProvider.prototype = {
    ...
    init(options){
      ...
      if (auth.id_token){
        try {
          ...
          // console.debug(this.username, this.auth.id);
    ...
    login(params){
      return $.promise('Login',  async resolve => {
        const searchParams = new URLSearchParams(window.document ? document.location.search : '');
        if (params){
        ...
        if (searchParams.has('code')){
          // return console.error(searchParams.get('code'));
          await this.getAccessToken({code: searchParams.get('code')});
        }
        resolve(this.init());
    ...
    logout(params){
      ...
      if ($().storage('id_token')) {
        ...
        // $.nav.reload();
        $.nav.reload($().url('https://login.aliconnect.nl/api/oauth').query({
          prompt: 'logout',
          client_id: $().client_id || '',
          redirect_uri: document.location.origin,
        }).toString());
    ...
    refreshToken(){
      ...
      this.refreshTokenHandle = new $.Client('https://login.aliconnect.nl/api/token').post({
        ...
        // };
        // var token = $.auth.access_token = $.cookie.access_token || $.cookie.id_token;
        // var access = JSON.parse(atob(token.split('.')[1]));
        // var time = new Date().getTime()/1000;
        // var expires_in = Math.round(access.exp - time);
        // console.error('RRRRRRRRRRRRefreshToken', expires_in, access);
    ...
    trackLocalSession(){
      ...
      arguments.callee.timeout = setTimeout(arguments.callee, TRACK_LOCAL_SESSION_TIME);
    },
    trackSession(){
      return;
      ...
      // alert(1);
      arguments.callee.timeout = setTimeout(arguments.callee, TRACK_SESSION_TIME);
    ...
    setUserstate(userstate){
      ...
      if (this.access && this.access.sub){
        if (userstate === 'available'){
          this.stateTimeout = setTimeout(() => $.setUserstate('inactive'), 5 * $.MIN_MS);
        } else if (userstate === 'inactive'){
          this.stateTimeout = setTimeout(() => $.setUserstate('appear_away'), 5 * $.MIN_MS);
        }
  ...
  Nav.prototype = {
		undo: function() {
			//console.debug('UNDO', private.updateList);
			if (this.undoData = private.updateList.shift()) {
		...
		update: function(data, targetItem, eventType) {
			...
			if (data.Value) {
				//var updateAction = '';
				if (!data.from) private.updateList.unshift(data); // Add data to update history list
		...
		cancel: () => {
			//console.debug(private.items.oncancel);
			return private.items.oncancel && private.items.oncancel.length ? private.items.oncancel.pop()() : null;
		},
		oncancel(fn) {
			const oncancel = private.items.oncancel = private.items.oncancel || [];
  ...
  Request.prototype = {
    ...
    exec(){
      for ([key, value] of this.URL.searchParams) {
        ...
        if (typeof $[key] === 'function'){
          // $.replaceUrl(new $().url().query(req.query).toString());
      ...
      if (path && path !== '/') {
        ...
        if ($().paths) {
          ...
          if (apiPath && apiPath.get){
            ...
            $().api(path).query(this.URL.searchParams.toString())
      ...
      if (!param) {
        ...
        if (['Object','Array'].includes(param.constructor.name)){
          if (formData) {
            ...
            this.input.data = $.JSON.stringify(param);
          }
    ...
    onload(event){
      ...
      if ($.LOG.HTTP && event.target.status < 400 || isModule){
        console.debug (
          // event.target.sender,
  ...
  WebSocket.prototype = {
    ...
    message(par){
      ...
      if (par){
        ...
        // req.device_id = $.cookie ? $.cookie.device_id : 'test_max';
      }
  ...
  Aim = function Aim (selector, context){
    ...
    if (window.Element && selector instanceof window.Element){
      if ($.has(selector.id)) return $.get(selector.id);
      selector = new Elem(selector, ...[...arguments].slice(1));
      if (selector.elem.id){
        $.set(selector.elem.id, selector);
      }
      return selector;
    }
    // if(!(this instanceof $)) return new $(...arguments);
  ...
  Aim.prototype = {
    ...
    progress(value = 0, max = 0) {
      if ($.statusbar) {
        value = $.statusbar.progress.elem.value = ($.statusbar.progress.elem.value || 0) + value;
        max = $.statusbar.progress.elem.max = ($.statusbar.progress.elem.max || 0) + max;
        $.statusbar.progress
        .max(max)
        .value(value || null)
    ...
    storage(selector, context, type){
      ...
      $.cookie = $.cookie || new Map(window.document ? document.cookie.split("; ").map(val => val.split('=')) : null);
      // console.debug($.cookie.get('id_token'));
    ...
    status(selector, context){
      if ($.statusbar && $.statusbar[selector]){
        $.statusbar[selector].attr('context', context);
      } else {
    ...
    translate(){
      ...
      .get()
      .then(event => {
        $.translate = new Map(Object.entries(event.body))
      });
    },
    tree(selector) {
    ...
    ws(options){
      return this.get(WebSocket, options ? Object.assign(options,{authProvider: options.authProvider || this.authProvider()}) : null);
    },
    _sendNotification() {
      ws.send({
        to: this.item.users,
    ...
    web() {
      ...
      // function require(){};
      private.openItems = private.openItems ? private.openItems.split(',') : [];
      ...
      .on('beforeunload', event => {
        if (private.handles) for (var name in private.handles) { private.handles[name].close(); }
      })
      ...
      .on('blur', event => {
        ...
        clearTimeout(private.stateTimeout);
      ...
      .on('click', event => {
        ...
        private.clickPath = event.path = event.path || function(el) {
          var path = [];
          while (el) {
            path.push(el);
            el = el.parentElement;
      ...
      .on('dragenter', event => {
        if (
          ...
          if (targetItemElement instanceof Element) {
            event.stopPropagation();
            setTimeout(() => (private.targetItemElement = targetItemElement).setAttribute('target', ''));
          }
        }
      })
      .on('dragleave', event => {
        if (private.targetItemElement) {
          private.targetItemElement.removeAttribute('target');
        }
      })
      .on('dragover', event => {
        if (private.targetItemElement) {
          $().status('target', private.targetType = eventKeyState(event));
      ...
      .on('dragstart', event => {
        ...
        if (elem) {
          ...
          var dragItems = $.nav.items.includes(item) ? $.nav.items : [item];
      ...
      .on('focus', event => {
        ...
        if ($().authProvider().access_token) {
          ...
          if (access.nonce) {
            $().url('https://login.aliconnect.nl/api/oauth').headers('Authorization', 'Bearer ' + $().authProvider().access_token).post({
      ...
      .on('keydown', event => {
        switch (event.keyPressed) {
          case 'Escape': {
            ...
            if ($.nav.clipboardItems && $.nav.clipboardItems.length) {
            ...
            if (private.iframeElement) {
              private.iframeElement.remove();
            }
      ...
      .on('keydown', event => {
        ...
        event.previousKey = private.previousKey;
        private.previousKey = event.keyPressed;
      }, true)
      .on('message', event => {
        ...
        if (data.itemID && data.attributeName && ('value' in data)) {
          ...
          if (window.meshitems && window.meshapi.item[data.itemID] && data.attributeName == 'err') {
            for (var i = 0, c = private.err.children, elErrRow; elErrRow = c[i]; i++) if (elErrRow.meshitem.src.itemID == data.itemID) break;
            if (elErrRow) {
              elErrRow.elEnd.innerText = (elErrRow.end = new Date()).toISOString().substr(11, 8);
              elErrRow.refresh();
            }
            else with (private.err.insertBefore(elErrRow = private.err.createElement('DIV', { className: 'row err start', itemID: data.itemID, meshitem: window.meshapi.item[data.itemID], start: new Date() }), private.err.firstChild)) {
              ...
              elErrRow.refresh = function() {
                if (this.end && this.accept) {
                  ...
                  private.err.removeChild(this);
                }
                private.errCont.style = private.err.children.length ? '' : 'display:none;';
      ...
      $().on('ready', async event => {
        ...
        $.prompt({
          ...
          help() {
            ...
            .assign('onsubmit', event => event.preventDefault() || this.searchHelp(event.target.search.value))
            .append(
              $('h1').text('Help'),
            )
            .properties({
            ...
            .append(
              $('button').class('abtn icn').text('ok').type('submit').attr('default', ''),
              this.resultList = $('div').class('col'),
            )
            // .prompts('docs','terms_of_use','privacy_policy','cookie_policy')
          },
          config() {
          ...
          share_item() {
            ...
            this.is.text('').append(
              ...
              }).on('submit', event => $().api('/' + item.tag).query('prompt', 'share_item').post(event).then(event => {
          ...
          account_delete_domain() {
            this.is.text('').append(
              ...
              $('form').class('col').properties({
                ...
                next: { type:'submit', default:1 },
              }).on('submit', event => $().api('/').query('prompt', 'account_delete_domain').post(event).then(event => {
                console.log(event.body);
                if (event.body === 'code_send') {
                  this.is.text('').append(
                    $('h1').ttext('prompt-sms_verification_code-title'),
                    ...
                    $('form').class('col').properties({
                      ...
                      next: { type:'submit', default: true, tabindex: 2 },
                    }).on('submit', event => $().api('/' + item.tag).query('prompt', 'account_delete_domain').post(event).then(event => {
                      this.is.text('').append(
          ...
          account_overview() {
            ...
            $().api('/').query('prompt', 'account_overview').get().then(event => {
    ...
    dashboard() {
      const panel = $('div').panel();
      $().api('/').query('request_type', 'personal_dashboard_data_domain').get().then(event => {
        ...
        async load() {
          ...
          $(document.body).class('col aim om bg').id('body').append(
            $.elemNavtop = $('header').id('navtop').class('row top bar noselect np').append(
              $().elemMenu = $('a').class('abtn icn menu').on('click', event => {
                if ($().elemMenuList && $().elemMenuList.style()) {
                  $().elemMenuList.style('');
                } else {
                  if ($().elemMenuList) $().elemMenuList.style('display:none;');
          ...
          if ($().authProvider().sub) {
            ...
            if ($.user = await $(`/Contact(${$().authProvider().sub})`).details()) {
              $().tree($.user);
              await $().api(`/`).query('request_type','visit').get().then(event => $().history = event.body);
              $.accountElem.item($.user, 'accountElem');
              $.user.emit('change');
    ...
    account_config(config, extend, save) {
      ...
      function upload() {
        ...
        $().api('/').post(panel).then(event => {
          console.debug("API", event.target.responseText );
        });
        // $().api(url).post(page).query({
  ...
  Object.assign(Aim, {
    ...
    MIN_MS: 60000,
    TRACK_LOCAL_SESSION_TIME: 5000, // timeout between tracking local cookie login session
    TRACK_SESSION_TIME: 30000, // timeout between tracking login session
    LIST_ATTRIBUTES: 'header0,header1,header2,name,schemaPath,Master,Src,Class,Tagname,InheritedID,HasChildren,HasAttachements,State,Categories,CreatedDateTime,LastModifiedDateTime,LastVisitDateTime,StartDateTime,EndDateTime,FinishDateTime',
    TAGNAMES: 'a,abbr,address,area,article,aside,audio,b,base,bdi,bdo,blockquote,body,br,button,canvas,caption,cite,code,col,colgroup,data,datalist,dd,del,details,dfn,dialog,div,dl,dt,em,embed,fieldset,figcaption,figure,footer,form,h1,h2,h3,h4,h5,h6,h7,h8,h9,head,header,hgroup,hr,html,i,iframe,img,input,ins,kbd,label,legend,li,link,main,map,mark,menu,meta,meter,nav,noscript,object,ol,optgroup,option,output,p,param,picture,pre,progress,q,rp,rt,ruby,s,samp,script,section,select,slot,small,source,span,strong,style,sub,summary,sup,table,tbody,td,template,textarea,tfoot,th,thead,time,title,tr,track,u,ul,var,video,wbr'.split(','),
    String: {
      keywords: {
        js: {
          reserved: {
      ...
      prg(line, lang, modifier){
        ...
        lang.forEach(lang => {
          if (this.keywords[lang]){
            for (let [groupName, group] of Object.entries(this.keywords[lang])){
      ...
      yaml(line){
        return this.replaceLines(this.prg (line, ['yaml']), {
          '(^#.*)': '<span class="hl-comt">$1</span>',
          '(:\\s|-\\s)(.+)': '$1<span class="hl-string">$2</span>',
          // '(?=\\s|^|-\\s)([^:]*)(?=:$|: )': '<span class="hl-attr">$1</span>',
          // '(?=:\\s)(.+)': '<span class="hl-string">$1</span>',
          // '([0-9]+)': '<span class="hl-nr">$1</span>',
        });
        // return line
        // .replace(/(#.*)/g, '<span class="hl-comt">$1</span>')
        // // .replace(/(# .*?$)(?!(.(?!<span))*<\/span>)/g, '<span class="hl-comt">$1</span>')
        // .replace(/(.*)([^:]+)(:)/g, '$1<span class="hl-attr">$2</span>$3')
        // .replace(/(:\s)(.*)/g, '$1<span class="hl-string">$2</span>')
        // // .replace(/(: |- )([^<>]+)(?![^<>]*>)(?!(.(?!<span))*<\/span>)/, '$1<span class="hl-string">$2</span>')
        // ;
      },
      json(line){
      ...
      st(line){
        return this;
        console.warn(this);
        return this.prg(line, ['st']);
      },
      html(line){
        return this.prg(line, ['html']);
        return this.prg(line, ['html','js','css']);
      },
      xml(line){
        return param.replace(
          line
          .replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\t/g, '  ')
          .replace(/(&lt;!--)/g, '<span class="hl-comt">$1').replace(/(--&gt;)/g, '$1</span>')
          ,
          {
            '(&lt;/)([^&]+)(&gt;)': '$1<span class="hl-tag">$2</span>$3',
            '(&lt;)([^\\s|^&]+)(\\s|&)': '$1<span class="hl-tag">$2</span>$3',
            '(")([^"]*)(")': '$1<span class="hl-string">$2</span>$3',
            '(\\s)([^=\\s]+)(=)': '$1<span class="hl-attr">$2</span>$3',
          }
        );
      },
      http(line){
        return line
        .replace(/^([^:| ]+)(?=:)(?!(.(?!<span))*<\/span>)/g, '<span class="hl-attr">$1</span>')
        ;
      },
      replaceLines(content, params, modifier){
        return content;
        return content.split(/\n/).map(line => {
          return this.replace(line, this, modifier);
        }).join('\n');
      },
      ...
      md(s){
        if (!s) return '';
  			...
  			var tag = {
  				level:[],
  				ident: function(tagName, add){
  					$newlines.push('  '.repeat(tag.level.length) + `<${tagName+(add?' '+add:'')}>`);
  					tag.level.push(tagName);
  				},
  				unident: function(){
  					var tagName = tag.level.pop();
  					$newlines.push('  '.repeat(tag.level.length) + `</${tagName}>`);
  				}
  			};
  			var toBegin = function() {
  				if (listlevel) {
  					$newlines[$newlines.length - 1] += "</li>";
  					while (listlevel > 1) {
  						tag.unident();
  						$newlines.push('  '.repeat(tag.level.length) + '</li>');
  						listlevel--;
  					}
  					listlevel = 0;
  				}
  				while (tag.level.length) {
  					tag.unident();
  				}
  			};
  			var toHTML = s => {
          ...
          .forEach(line => {
  					...
  					var newLine = '';
  					if (identLevel > ident) {
  						while (identLevel > ident) {
  							html[html.length-1] += '</LI>';
  							html.push('  '.repeat(identLevel) + '</' + (identTag[identLevel--]) + '>');
  						}
  					}
  					if (!line) {
  						setTag('');
  						...
  						setTag('');
  						var listTag = listMatch[2].match(/([0-9]. )/) ? 'OL' : 'UL';
  						if (identLevel === ident) {
  							html[html.length-1] += '</LI>';
  						} else if (identLevel < ident) {
  							html.push('  '.repeat(ident) + '<' + (identTag[ident] = listTag) + '>');
  						}
  						identLevel = ident;
  						html.push(
  							'  '.repeat(ident + 1)
        ...
        return s.replace(/(.*?)(```.*?```|$)/gs, (s,md,codeString) => toHTML(md) + toCodeHtml(codeString));
      },
    },
    Date: {
      toString(){
        let args = [...arguments];
        // console.debug(args, args[0], String(args[0]), args[1], String(args[1]));
        for (let i = 0, datetime, args = [...arguments]; datetime = args[i]; i++){
          // if (datetime === 'Invalid Date') console.error(datetime);
          datetime = String(datetime);
          // if (datetime === 'undefined') console.error(datetime);
          if (datetime){
            let date = new Date(datetime.replace('000Z','Z'));
            // console.debug('datatime',datetime, date);
            return date.toDateText();
          }
        }
        return '';
      },
    ...
    Object: {
      toJs(data){
        return JSON.stringify(event.body, null, 2).replace(/"(\w+)":\s/gs,'$1: ');
      },
      children(selector){
        if (selector.children) return selector.children;
        if (selector.items) return selector.items;
        if (typeof selector === 'object'){
          return Object.entries(selector).map(entry => Object.assign(entry[1], {name: entry[0]}))
        }
      },
      attributes(selector){
        return Object.fromEntries(Object.entries(selector).filter(entry => !['function', 'object'].includes(typeof entry[1])))
      },
      methods(selector){
        return Object.fromEntries(Object.entries(selector).filter(entry => typeof entry[1] === 'function'))
      },
    },
    JSON: {
      stringify (data){
        ...
        (function recurse(parent, selector, context){
          if (parent && selector && selector instanceof Object){
            for (let [key, value] of Object.entries(selector)){
              ...
              } else if (typeof value !== 'object' || Array.isArray(value) || !(key in parent) || objects.includes(value)){
                parent[key] = value;
              } else {
                objects.push(value);
    ...
    nav: new Nav(),
    oisImg(ofile) {
			return (ofile.type || '').indexOf('image') != -1 || $.isImgSrc(ofile.src)
		},
    translate: new Map(),
  ...
  Item = Object.assign(function () {}, {
    ...
    new(item){
      return $.promise( 'New item', resolve => {
        ...
        //var schema = $.components.schemas[this.caller.schema];// || $.components.schemas[schemaname = api.find(post.classID)];
        //var post = { id: a.shift(), };
  ...
  Item.prototype = {
      ...
      return $.promise( 'Attribute', async resolve => {
        try {
          ...
          function getItem(selector) {
            ...
            if ($.has(selector)) return $.get(selector);
          }
          if (value.target) {
          ...
          if (!private.noPost && postdata){
            ...
            if (values.length){
              ...
              private.itemsModifiedResolve = private.itemsModifiedResolve || [];
              private.itemsModifiedResolve.push([resolve, item]);
              private.itemsModifiedTimeout = setTimeout(() => {
                ...
                private.itemsModifiedResolve.forEach(([resolve, item]) => resolve(item));
                private.itemsModifiedResolve = [];
              });
            }
          } else {
            resolve(item);
          ...
          // return;
          if (property.type === 'datetime'){
            if (value.Value && value.Value.match(/T[0-9][0-9]:[0-9][0-9]$/)){
              value.Value = (value.Value + ':00').substr(0,19);
            }
          }
    ...
    clone() {
      if (this.data.Src) {
        return $.promise( 'Clone', resolve => {
          ...
          $().api(`/${this.tag}`).query('request_type','build_clone_data').get().then(async event => {
              ...
              if (targetId && sourceId) {
                ...
                await children.forEach(async (child,i) => {
                  ...
                  if (child.cloneId === null){
                    ...
                    await $().api(`/${schemaName}`).input(data).post().then(event => clone(event.body.data.ID, child.itemId));
                  } else {
    ...
    delete(){
      this.remove();
      return $().api(`/${this.tag}`).delete();
    },
    displayvalue(attributeName) {
      return $.displayvalue(this.getValue(attributeName), ((this.schema||{}).properties||{})[attributeName]);
    },
    get elements(){
    ...
    get hasImage(){
      return this.hasAttach && Files.oisImg(this.files[0]);
    },
    headerId(id) {
  ...
  $().on({
    ...
    // logout(event){
    //   $.nav.reload('https://login.aliconnect.nl/api/oauth?' + new URLSearchParams({
    //     prompt: 'logout',
    //     client_id: config.auth.client_id || config.auth.clientId,
    //     redirect_uri: document.location.origin,
    //   }).toString());
  ...
  Doc.prototype = {
      ...
      // console.log('FOCUS', newFocusElement, this.focusElement);
			if (newFocusElement && newFocusElement !== this.focusElement) {
				if (event && event.shiftKey) {
					...
					$.nav.setItem(all.map(elem => $(elem).item()), 'checked', '');
				} else {
            ...
            // //console.log(newFocusElement);
						if (newFocusElement) {
							...
							if (newFocusElement.item) {
								$.nav.setItem([newFocusElement.item], 'checked', '');
							}
						}
    ...
    refilter() {
        ...
        for (let [attributeName, attribute] of Object.entries(this.activeFilterAttributes)) {
          if (!(attributeName in row.filterfieldslower) || attribute.indexOf(row.filterfieldslower[attributeName]) === -1) {
			...
			// 			//var schema = $.components.schemas[className];
			...
			// 			////menuitem.onclick = $.components.schemas.item.add.bind({ schema: className, srcID: childclass.scrID, masterID: childclass.masterID, });
			// 			childclass.onclick = $.components.schemas.item.add.bind({ schema: childclass.schema || childclass.Title, masterID: item ? item.id : null });
              ...
              [...$().schemas().entries()].map(
                ...
                .on('click', event => {
                  ...
                  $().copyFrom(sourceItem, targetItem).then(item => {
					...
					$('button').class('abtn refresh').on('click', event => $().api(document.location.pathname.replace(/\/id\/.*/,'')).query(document.location.search).get().then(event => $().list(event.body))),
    ...
    async show(items, path) {
        ...
        this.tag = ((this.items.url||'').match(/\w+\(\d+\)/)||[''])[0];
          ...
          // }
				...
				if ($.navtree) {
					if ($.navtree.elSelected) $.navtree.elSelected.removeAttribute('selected');
					if (private.folder) {
						var item = $.getItem(private.folder.substr(1).split('?').shift());
    ...
    sortlist(a, b) {
      ...
      if (a.searchname && b.searchname) {
        ...
        ia = 999,
        ib = 999,
        l = private.search.value.length;
        for (var i = 0, word; word = awords[i]; i++) {
          //// //console.debug(word);
          if (word.indexOf(private.search.value) != -1) ia = Math.min(ia, word.indexOf(private.search.value) + word.length - l);
          //var pos = word.indexOf(private.search.value);
          //ia -= pos != -1 ? l + word.length - pos : 0;
        }
        for (var i = 0, word; word = bwords[i]; i++) {
          //// //console.debug(word);
          if (word.indexOf(private.search.value) != -1) ib = Math.min(ib, word.indexOf(private.search.value) + word.length - l);
          //var pos = word.indexOf(private.search.value);
    ...
    table() {
      ...
      if (item.elemListLi) {
        item.elemListLi
			...
			// }
			//if (this.hasModified = !$.nav.replace[this.id] || new Date($.his[this.id]) < new Date(this.modifiedDT)) createElement('DIV', { className: !$.his[this.id] ? 'created' : 'modified' });
    ...
    Object.assign(Item.prototype, {
      ...
      focus(event) {
        ...
        if (self.focusElement && self.focusElement.elemTreeLi) {
          ...
          // $.nav.items.push(self.focusElement);
        }
        // $.nav.cancel();
        ...
        if (self.focusElement.elemTreeLi) {
          ...
          //       selactive ^= 1;
          //       if ($.nav.items.indexOf(listItemElement.item) === -1) {
          //         $.nav.items.push(listItemElement.item);
          //       }
          //     }
          ...
          //       }
          //     }
          //     if (selactive && $.nav.items.indexOf(listItemElement.item) === -1) {
          //       $.nav.items.push(listItemElement.item);
          //     }
          //   });
          //   $.nav.items.forEach(listItemElement => {
      ...
      select(event) {
        ...
        document.location.href = `#/${this.tag}/children/id/${btoa(this['@id'])}?$select=${AIM.LIST_ATTRIBUTES}&$filter=FinishDateTime+IS+NULL`;
      ...
      async open(event) {
		...
		$(elem).extend({
			...
			// 	}
			// 	if ($.elCover) private.body.removeChild($.elCover);
    ...
    childnode(child) {
      ...
      .open(private.openItems.includes(child.tag))
      .item(child, 'treeview')
  ...
  'checked,disabled,hasChildren,selected'.split(',').forEach(name => Elem.prototype[name] = function attrIfTrue(value) {
    return this.attr(name, value ? '' : null)
  });
  'accept,accesskey,action,align,allow,alt,async,autocapitalize,autocomplete,autofocus,background,bgcolor,border,buffered,capture,challenge,charset,cite, class,code,codebase,color,cols,colspan,content,contenteditable,contextmenu,controls,coords,crossorigin,csp,data,datetime,decoding,defer,dir,dirname,displayvalue,download,enctype,enterkeyhint,for,form,formaction,formenctype,formmethod,formnovalidate,formtarget,headers,height,hidden,high,href,hreflang,hotkey,icon,id,importance,integrity,intrinsicsize,inputmode,ismap,itemprop,keytype,kind,label,lang,language,loading,list,loop,low,manifest,max,maxlength,minlength,media,method,min,multiple,muted,name,novalidate,open,optimum,pattern,ping,placeholder,poster,preload,radiogroup,readonly,referrerpolicy,rel,required,reversed,rows,rowspan,sandbox,scope,scoped,shape,size,sizes,slot,span,spellcheck,src,srcdoc,srclang,srcset,start,step,style,summary,tabindex,target,tag,title,translate,type,usemap,value,width,wrap'.split(',').forEach(name => Elem.prototype[name] = function attrValue() {
    return this.attr(name, ...arguments);
  });
  'click'.split(',').forEach(name => Elem.prototype[name] = function exec() {
    this.elem[name](...arguments);
    return this;
  });
  'submit'.split(',').forEach(name => Elem.prototype[name] = function emit() {
  ...
  Object.assign(Elem.prototype, {
    ...
    append() {
			...
			args.forEach(a => !a ? null : this.elem.append(typeof a === 'string' ? document.createTextNode(a) : a.elem || a));
			return this;
		},
    accept_scope(scope, socket_id) {
      ...
      properties.expire_time = {format: 'number', value: 3600};
      const form = $().promptform($().api('/oauth').query('socket_id', socket_id), this.elem, arguments.callee.name, {
        properties: properties,
        btns: {
          deny: { name: 'accept', value:'deny', type:'button' },
          allow: { name: 'accept', value:'allow', type:'submit', default: true },
    ...
    checkbox() {
      ...
      const id = 'checkbox' + ($.checkboxInt = $.checkboxInt ? ++$.checkboxInt : 1);
      return [
        this
        .class('check')
        .attr('id', id)
    ...
    cookieWarning(){
      if (!window.localStorage.getItem('cookieWarning')) {
        const elem = $('div')
        .parent(document.body)
        .class('cookieWarning')
        .text('Indien u gebruik maakt van deze site gaat u akkoord met onze cookie voorwaarden')
        .append(
          $('button')
          .text('Akkoord')
          .on('click', event => {
            window.localStorage.setItem('cookieWarning', 'done');
            elem.remove();
          })
        )
      }
    ...
    css(selector, value) {
			...
			if (selector instanceof Object) {
				...
				if (value === undefined) {
					elem.style.cssText += selector;
				} else {
					elem.style.cssText += `;${selector}:${value};`;
    ...
    editor(lang) {
      ...
      const html = $.String.js(elem.innerText);
      // console.log(html);
			return this
      ...
      .text('')
			...
			.on('keyup', e => {
				if (e.keyCode >= 0x30 || e.keyCode == 0x20) {
					...
					// js(el);
					elem.innerHTML = $.String.js(elem.innerText);
					setCaret(pos, elem);
				}
			})
        ...
        .then(event => {
          ...
          if (event.body.type === 'application/pdf') {
            $().pdfpages(event.body.src).then(pages => {
              ...
              $().api(`/${this.item.tag}/?request_type=words`).patch(words).then(event => {
    ...
    itemAttr(items, attributeName, value) {
			...
			const a = private.attributeItems = private.attributeItems || {};
			if (a[attributeName]) {
      ...
      [...arguments].filter(Boolean).forEach(properties => {
          ...
          selectitem: {
            view() {
              ...
              .filter(item => this.schema && (this.schema === '*' || this.schema.includes(item.schemaName)));
  						const listElement = private.listElement = private.listElement || $('datalist')
              .parent(document.body)
              .id('listitems')
              .on('updateList', event => {
                listElement.text('');
              ...
              selector.append(
                  ...
                  .on('keyup', event => {
                    ...
                    this.timeout = setTimeout(() => {
                      ...
                      .get()
                      .then(event => {
                        private.listElement.updateList(property.schema, value, this.request = null);
                      });
                    },500);
                  }),
  								$('label').text(property.title || property.name),
        ...
        for (let [name, property] of Object.entries(properties)) {
          ...
          Property.prototype = Object.create(property, {
            displayvalue: {
              get() {
                return $.displayvalue(this.value, property);
              },
            },
          });
          new Property();
    ...
    _select(event) {
			...
			const setOpen = open => {
				...
				$(elem).attr('open', open);
				if (elem.label) {
					var foldersOpen = $.cookie.foldersOpen
					? $.cookie.foldersOpen.split(', ').filter(x => x !== elem.label)
					: [];
					if (open) {
    ...
    show(item, doEdit) {
      ...
      item.details().then(event => {
        ...
        function logVisit() {
          if (item.data.ID) {
            ...
            $.viewTimeout = setTimeout(() => {
              $().api('/').query('request_type','visit').query('id',item.data.ID).get().then(event => {
        ...
        function breakdown_data() {
          return $().api(`/${item.tag}`).query('request_type', 'build_breakdown').get().then(event => {
        ...
        function build_map(fn) {
          if (itemdata.build_map) {
            ...
            $().api(`/${item.tag}`).query('request_type', 'build_breakdown').get().then(event => {
              const data = event.body.value;
              let items = [];
              (function row(item, level) {
                item.level = level;
        ...
        this.text('').append(
          $('nav').class('row top abs btnbar np').append(
            ...
            $('button').class('abtn view').append($('ul').append(
              ...
              $('li').class('abtn model3d').text('Build 3D Model').on('click', event => {
                const elem = $('div').parent($('list')).class('col abs').append(
                  ...
                  this.three = $('div').class('col aco').three(
                    this.init = three => (this.rebuild = event => $().api('/'+item.tag).query('three', '').get().then(three.build))()
                  ),
                );
              }),
              $('li').class('abtn network').text('Netwerk').on('click', event => {
                (function init() {
                  ...
                  $().api(`/${item.tag}`).query('request_type','build_link_data').get().then(
            ...
            $('button').class('abtn close').name('close').on('click', event => {
              this.text('');
              delete ItemSelected;
              $.replaceUrl(document.location.pathname.replace(/\/id\/.*/,'')+'?'+document.location.search);
            }),
          ),
          this.header(item),
          this.main = $('main')
          ...
          .on('submit', event => {
            ...
            $().api(`/${item.tag}/Messages`).post(event.target).then(event => this.showMessages());
            return false;
          })
          .append(
            // $().files(),
    ...
    showpage(item) {
      item.details().then(item => {
        ...
        $().api(`/${item.tag}/children`).select('*').get().then(async event => {
          console.log(event);
          this.elemDiv.append(
    ...
    showLinks(item) {
			$().api(`/${item.tag}`).query('request_type','build_link_data').get().then(event => {
				//console.log(event.body);
    ...
    statusbar() {
      $.statusbar = this.class('row statusbar np').append(
        ['ws','aliconnector','http','checked','clipboard','source','target','main'].map(name => this[name] = $('span').class(name)),
      );
    ...
    window(event) {
			...
			if (private.handles[this.url]) {
				private.handles[this.url].focus();
			}
			else {
				private.handles[this.url] = window.open(this.url, this.url, 'width=600, height=800, left=' + (event.screenX || 0) + ', top=' + (event.screenY || 0));
				private.handles[this.url].name = this.url;
				private.handles[this.url].onbeforeunload = function() { private.handles[this.name] = null };
			}
		},
		sampleWindow(url) {
			const height = 600;
